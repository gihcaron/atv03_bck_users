...
const updatePost = async  (id, user_id, autor, likes, comentarios, salvamentos, compartilhamentos, imagem, photo) => {
    const result = await pool.query(
        `UPDATE posts 
         SET user_id = $1, autor = $2, likes = $3, comentarios = $4, salvamentos = $5, compartilhamentos = $6, imagem = $7, photo = $8 
         WHERE id = $9 
         RETURNING *`,
        [user_id, autor, likes, comentarios, salvamentos, compartilhamentos, imagem, photo, id]
    );
    return result.rows[0];
};
...
const deletePost = async (id) => { 
    ...
    return { message: "Post deletado com sucesso!", post: result.rows[0] };
};
